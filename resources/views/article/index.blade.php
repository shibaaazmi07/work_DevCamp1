<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <a href="{{ route('article.create') }}" class="border border-green-500 
                hover:bg-green-500 hover:text-white px-4 py-2 rounded-md">CREATE</a>
        </div>
                    ...
                    <table class="border-collapse table-auto w-full text-sm">
                        ...
                        <tbody class="bg-white">
                            ...
                            @foreach ($articles as $article)
                                <tr>
                                    {{-- bisa menggunakan {{ $article['title'] }} $article ->title dan $article['description']  --}}
                                    ...
                                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">
                                     ...
                                     <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">
                                        ...
                                        <a href="{{ route('article.edit', $article->id) }}" class="border border-yellow-500 
                                            hover:bg-yellow-500 hover:text-white px-4 py-2 rounded-md">EDIT</a>

                                        <form method="post" action="{{ route('article.destroy', $article->id) }}" class="inline">
                                            @csrf
                                            @method('delete')
                                            <button class="border border-red-500 hover:bg-red-500 hover:text-white px-4 py-2 
